# Contexts and Backend Architecture

Source of truth for `contexts/`, dependency injection, repositories, application services, error handling, and server actions.

In this repo that tree lives at `src/contexts/`. Guideline paths written as `contexts/...` and imports written as `@/contexts/...` both mean `src/contexts`.

Portfolio routes, `src/ui` composition, case studies, and motion stay in [architecture.md](./architecture.md). Visual rules stay in [ui-guidelines.md](./ui-guidelines.md). Use this document for anything that crosses into domain logic or an external system.

Static portfolio content (case studies, profile, experience, testimonials) stays in `src/content` and `src/lib/content`. Contact submission, the AI assistant, and any other mutation or integration go through a context: hook or server action, then application service, then repository.
